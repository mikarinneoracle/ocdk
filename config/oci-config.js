...
`;
/** Ensure tail-function-logs.js exists in projectDir; remove legacy JS script and write fresh file based on scripts/tail-function-log.js. Returns path. */
function ensureTailFunctionLogsScript(projectDir) {
    const p = path.join(projectDir, 'tail-function-logs.js');
    try {
        if (fs.existsSync(p)) {
            fs.unlinkSync(p);
        }
    ...
    try {
        const templatePath = path.join(__dirname, '..', 'scripts', 'tail-function-log.js');
        if (!fs.existsSync(templatePath)) {
            throw new Error('missing scripts/tail-function-log.js in ocdk package');
        }
        const content = fs.readFileSync(templatePath, 'utf8');
        fs.writeFileSync(p, content, 'utf8');
    }
    catch {
        // If we cannot read the template for some reason, leave the script absent.
    }
    return p;
}
